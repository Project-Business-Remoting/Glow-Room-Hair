...
import { SERVICES_FEMMES, SERVICES_HOMMES, POLITIQUES } from './data.js';
...
function _bindEvents(section) {
  ...
  section.addEventListener('click', e => {
    ...
    section.querySelectorAll('.service-card__variant.is-selected')
    ...
    document.dispatchEvent(new CustomEvent('app:service-selected', {
      detail: {
  ...
  section.addEventListener('keydown', e => {
    ...
    const variant = e.target.closest('.service-card__variant');
    if (variant) { e.preventDefault(); variant.click(); }
  });
}
