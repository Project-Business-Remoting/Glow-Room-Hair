...
import {
  SERVICES_FEMMES, SERVICES_HOMMES,
  HORAIRES, METHODES_PAIEMENT, POLITIQUES,
} from './data.js';
import { onPageEnter, navigate } from './router.js';
...
function _buildServiceCard(service, genre) {
  ...
  const variants = service.variantes.map(v => {
...
function _bindEvents(root) {
  root.addEventListener('click', e => {
    ...
    switch (action) {
      ...
      case 'pay':      return _handlePayment(root);
...
function _validateS1() {
  if (!B.service) { _toast('Veuillez sélectionner un service.', 'info'); return; }
  _goToStep(2);
}

function _validateS2() {
  if (!B.date) { _toast('Veuillez choisir une date.', 'info'); return; }
  if (!B.time) { _toast('Veuillez choisir un créneau horaire.', 'info'); return; }
  _goToStep(3);
}
...
function _buildCalGrid() {
  ...
  const header = DAY_ABBREVS.map(d =>
    `<span class="booking-calendar__day-name">${d}</span>`
  ).join('');

  const blanks = Array(offset).fill(
    `<span class="booking-calendar__day" aria-hidden="true"></span>`
  ).join('');
...
function _resetState() {
  Object.assign(B, {
    step: 0, service: null, date: null, time: null,
    name: '', phone: '', email: '', paymentMethod: null,
  });
